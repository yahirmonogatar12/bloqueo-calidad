...
{
    public async Task<Station?> GetByCodeAsync(string stationCode, CancellationToken ct = default)
    {
        const string sql = """
            SELECT id               AS Id,
        ...
        return await conn.QueryFirstOrDefaultAsync<Station>(sql, new { StationCode = stationCode });
    }
